...
// biome-ignore lint/complexity/useLiteralKeys: tsconfig noPropertyAccessFromIndexSignature requires bracket notation
const API_BASE_URL =
...
export async function getUsersByDojo(
  dojoId: string,
): Promise<ApiResult<readonly User[]>> {
  return apiFetch<readonly User[]>(
    `/api/users?dojoId=${encodeURIComponent(dojoId)}`,
  );
}
...
export async function getPractices(
  userId?: string,
): Promise<ApiResult<readonly Practice[]>> {
...
export async function getVideos(
  userId?: string,
): Promise<ApiResult<readonly Video[]>> {
  const qs = userId ? `?userId=${encodeURIComponent(userId)}` : "";
...
export async function getAnalyses(
  userId?: string,
): Promise<ApiResult<readonly Analysis[]>> {
  const qs = userId ? `?userId=${encodeURIComponent(userId)}` : "";
...
export async function getAnalysisByVideo(
  videoId: string,
): Promise<ApiResult<Analysis>> {
  return apiFetch<Analysis>(
    `/api/analyses/video/${encodeURIComponent(videoId)}`,
  );
}
...
export async function getReservations(
  dojoId?: string,
  date?: string,
): Promise<ApiResult<readonly Reservation[]>> {
...
export async function deleteReservation(
  id: string,
): Promise<ApiResult<{ readonly deleted: true }>> {
  return apiFetch<{ readonly deleted: true }>(
...
export async function getExamChecklists(
  userId?: string,
): Promise<ApiResult<readonly ExamChecklist[]>> {
  const qs = userId ? `?userId=${encodeURIComponent(userId)}` : "";
...
export async function getExamChecklist(
  id: string,
): Promise<ApiResult<ExamChecklist>> {
  return apiFetch<ExamChecklist>(
    `/api/exam-checklists/${encodeURIComponent(id)}`,
  );
}
...
export async function getDashboardSummary(
  ...
  return apiFetch<DashboardSummary>(
    `/api/dashboard/${encodeURIComponent(dojoId)}`,
  );
}
